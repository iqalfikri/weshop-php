<?php
    ...
    if($banner_id != "")
    {
		...
		$banner = $row["banner"];
		$link = $row["link"];
		$gambar = "<img src='". BASE_URL."img/slide/$row[gambar]' style='width: 200px;vertical-align: middle;' />";
		$keterangan_gambar = "(klik 'Pilih Gambar' hanya jika tidak ingin mengganti gambar)";
		$status = $row["status"];
    }   
?>

<form action="<?= BASE_URL."module/banner/action.php?banner_id=$banner_id"?>" method="post"
    ...
    <div class="element-form">
        <label>Gambar <?= $keterangan_gambar; ?></label>
