<?php  
    ...
    $query = mysqli_query($db, "SELECT pesanan.pesanan_id, pesanan.nama_penerima, pesanan.nomor_telepon, pesanan.alamat, pesanan.tanggal_pemesanan, user.nama, kota.kota, kota.tarif FROM pesanan JOIN user ON pesanan.user_id=user.user_id JOIN kota ON kota.kota_id=pesanan.kota_id WHERE pesanan.pesanan_id='$pesanan_id'");
...
<div id="frame-keterangan-pembayaran">
    <p>
        Silahkan Lakukan pemabayaran ke BANK ABC </br>
        Nomor Account : 0000 - 3432 - 8889 (A/N Weshop). </br>
        Setelah melakukan pembayaran silahkan lakukan konfirmasi pembayaran
        <a
            href="<?= BASE_URL."index.php?page=my_profile&module=pesanan&action=konfirmasi_pembayaran&pesanan_id=$pesanan_id"?>">Disini</a>
    </p>
</div>
